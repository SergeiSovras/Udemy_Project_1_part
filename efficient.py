...
class PageWithListings:
    ...
    def get_listings(self):
        ...
        return [Listing(div) for div in divs] #use the tuple

    def get_listings_high_to_low(self):
        return sorted(self.get_listings(), key=lambda x: x.wealth, reverse=True) #sorting

    @property
    def highest_wealth(self):
        return self.get_listings_high_to_low()[0] #show the first item with the highest value
